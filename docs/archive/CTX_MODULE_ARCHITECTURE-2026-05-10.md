> # ⛔ АРХИВ — ЗАМЫСЕЛ, НЕ РЕАЛИЗАЦИЯ
>
> **Архивировано 2026-09-18.** Это проектный документ, помеченный как «Утверждённая
> архитектура», но **ничего из описанного здесь в коде не реализовано**: Module Mode,
> UCIP v2, 4-слойный Context Package, Decision Engine и Context Surgery отсутствуют в
> `src/` полностью.
>
> Документ сохранён как источник исходного замысла. Актуальная и честная проработка того
> же направления — в `docs/proxy/`.
>
> **Актуально:** `docs/proxy/01-architecture.md` (прокси), `docs/proxy/02-memory-model.md`
> (Context Package), `docs/known-issues.md` (что мешает реализации).
