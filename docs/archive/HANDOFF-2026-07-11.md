...
>
> Этот документ **архивирован 2026-09-18** и содержит **проверенные ложные утверждения**.
> Он сохранён как история переписки между сессиями разработки, а не как описание
> состояния проекта.
>
> Известные ложные утверждения (проверено против кода):
>
> | Утверждение | Строка | Факт |
> |---|---|---|
> | «`qwen2.5/qwen3.5` в тестах — обновлено в `5aa4777`» | `:155` | Не выполнено: 12 падений из-за имён моделей |
> | «`run_stream()` решил задержку ~25 сек» | `:153` | `run_stream()` имеет ноль вызовов |
> | «`engine/` остался только в `__main__.py`» | `:160` | Достижим по HTTP через `api/state.py:7` |
> | «465 тестов проходят» (в `CTX_BOOK`) | — | 367 passed / 25 failed |
> | Внутренние противоречия | `:155` / `:171` / `:441` | «исправлено» / «надо чинить» / «19 падений» — фактически 25 |
>
> **Актуальный журнал разработки:** `docs/JOURNAL.md`
> **Актуальный реестр проблем:** `docs/known-issues.md`
